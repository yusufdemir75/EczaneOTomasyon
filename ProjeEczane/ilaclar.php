...
<table id="example" class="table bacground:#6c747e" style="width:100%;"  >
<a href="ekle-ilac.php"<button style="float: right" type="submit" class="btn btn-primary">Ürün Ekle</button></a>
<thead>
<tr>
    <th>ID</th>
    <th>Firma</th>
...
<tbody>
    <?php 
        $urunsor = $baglanti->prepare("SELECT * FROM ilaclar");
        $urunsor->execute();
        while ($uruncek = $urunsor->fetch(PDO::FETCH_ASSOC)) {
    ?>
    <tr>        
        <td><?PHP   echo $uruncek['id']?></td>
        <td><?PHP   echo $uruncek['uretici_firma']?></td>
        <td><?PHP   echo $uruncek['dozaj']?></td>
        <td><?PHP   echo $uruncek['fiyat']?></td>
        <td><?PHP   echo $uruncek['ilac_adi']?></td>
        <td><?PHP   echo $uruncek['kategori']?></td>
  
        <td><a href="stokstok-duzenle.php?id=<?php echo $uruncek['id'] ?>"><button type="submit" class="btn btn-warning">Stok Ekle</button></a></td>
        <td><a href="duzenle-ilac.php?id=<?php echo $uruncek['id'] ?>"><button type="submit" class="btn btn-dark">Düzenle</button></a></td>
        <td><a href="islem-ilac.php?urunsil&id=<?php echo $uruncek['id'] ?>"><button type="submit" class="btn btn-danger">Sil</button></td>
