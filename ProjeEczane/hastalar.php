...
<a href="ekle-hasta.php"<button style="float: right" type="submit" class="btn btn-primary">Hasta Ekle</button></a>
<thead>
<tr>
    <th>ID</th>
    <th>Adı</th>
...
<tbody>
    <?php 
        $urunsor = $baglanti->prepare("SELECT * FROM hasta");
        $urunsor->execute();
        while ($uruncek = $urunsor->fetch(PDO::FETCH_ASSOC)) {
    ?>
    <tr>        
        <td><?PHP   echo $uruncek['id']?></td>
        <td><?PHP   echo $uruncek['hasta_ad']?></td>
        <td><?PHP   echo $uruncek['hasta_soyad']?></td>
        <td><?PHP   echo $uruncek['cinsiyet']?></td>
        <td><?PHP   echo $uruncek['dogum_tarihi']?></td>
        <td><?PHP   echo $uruncek['adres']?></td>


        <td><a href="duzenle-hasta.php?id=<?php echo $uruncek['id'] ?>"><button type="submit" class="btn btn-dark">Düzenle</button></a></td>
        <td><a href="islem-hasta.php?hastasil&id=<?php echo $uruncek['id'] ?>"><button type="submit" class="btn btn-danger">Sil</button></td>
